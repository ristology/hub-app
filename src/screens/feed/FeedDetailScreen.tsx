...
export default function FeedDetailScreen() {
  ...
  // ── Edit komentar — long-press own komentar dalam window 15 menit ──
  const [editingKom, setEditingKom] = useState<{ id: number; original: string } | null>(null);
  const editCommentMutation = useMutation({
    mutationFn: ({ kid, text }: { kid: number; text: string }) => feedApi.editComment(id, kid, text),
    onSuccess: () => {
      setEditingKom(null);
      setKomentar('');
      queryClient.invalidateQueries({ queryKey: ['feed', id] });
  ...
  const handleLongPressKom = (k: FeedKomentar) => {
  ...
  const handleSubmitKomentar = () => {
    ...
    if (!t) return;
    if (editingKom) {
      editCommentMutation.mutate({ kid: editingKom.id, text: t });
    } else {
      ...
      <View style={{
        ...
        <View style={styles.inputBar}>
          ...
          <TouchableOpacity
            ...
            disabled={commentMutation.isPending || editCommentMutation.isPending || !komentar.trim()}
          >
...
function CommentItem({ k, bindRef, highlighted, onReply, onLongPress }: {
  k: FeedKomentar;
  ...
  return (
    <TouchableOpacity
      ...
      style={[styles.commentItem, highlighted && styles.commentHighlight]}
    >
